...
export default function App({ lang = 'fr' }) {
  ...
  function loadMyProfile(email, kind) {
    ...
    if (profile) {
      ...
      if (kind==='affilie' || profile.grade==='Postdoc') setAffectRows(getAffectationsByAffilie(email))
  ...
  async function saveChercheurAll() {
    ...
    // Postdoc : affectation obligatoire à 100%
    if (form.grade === 'Postdoc') {
      const total = affectationTotal(affectRows)
      if (total < 100) {
        showToast(lang==='fr'?`Affectation Postdoc incomplète : ${total}% sur 100% requis`:`Postdoc assignment incomplete: ${total}% of 100% required`, 'error')
        return
      }
      if (total > 100) {
    ...
    // Sauvegarder les affectations Postdoc
    if (form.grade === 'Postdoc') {
      setAffectations(email, affectRows.filter(r=>r.intitule||r.num_as))
    }
  ...
  if (view === 'profil_chercheur' || view === 'profil_affilie') {
    ...
    const isPostdoc  = form.grade === 'Postdoc'
    ...
    // Affectation obligatoire pour Affiliés ET Postdocs
    const needsAffectation = isAffilie || isPostdoc
    const totalAffect = needsAffectation ? affectationTotal(affectRows) : 0
    const overLimit   = totalAffect > 100
    const notComplete = needsAffectation && totalAffect < 100
    ...
    return (
      <div style={{ minHeight:'100vh', background:C.g1, fontFamily:'system-ui,-apple-system,sans-serif' }}>
        ...
        <div style={{ maxWidth:1100, margin:'0 auto', padding:'20px 16px 40px' }}>
          ...
          {needsAffectation && (
            <div style={{ background:'#fff', border:`1.5px solid ${overLimit?C.red:notComplete?C.amber:totalAffect===100?C.green:C.g3}`, borderRadius:12, padding:'16px 18px', marginBottom:14 }}>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12, flexWrap:'wrap', gap:8 }}>
                <div>
                  <h3 style={{ margin:0, fontSize:13, fontWeight:700, color:C.navy }}>
                    ...
                    {isPostdoc && <span style={{ marginLeft:8, fontSize:11, background:'#EFF6FF', color:C.blue, padding:'2px 8px', borderRadius:6, fontWeight:600 }}>Postdoc</span>}
                  </h3>
                  <p style={{ margin:'3px 0 0', fontSize:11, color:C.gt }}>
                    {lang==='fr'?"Le total doit obligatoirement atteindre 100%. Ajoutez des lignes jusqu'à atteindre 100%.":'Total must reach 100%. Add rows until you reach 100%.'}
                  </p>
                </div>
                <span style={{ fontSize:20, fontWeight:800, color:overLimit?C.red:totalAffect===100?C.green:C.amber }}>{totalAffect}%</span>
              </div>

              <div style={{ border:`1px solid ${C.g3}`, borderRadius:8, overflow:'hidden', marginBottom:10 }}>
                <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
                  <thead>
                    <tr style={{ background:C.navy }}>
                      <th style={{ padding:'8px 12px', color:'#fff', fontWeight:600, textAlign:'left', fontSize:12, width:130 }}>N° AS</th>
                      <th style={{ padding:'8px 12px', color:'#fff', fontWeight:600, textAlign:'left', fontSize:12 }}>{lang==='fr'?'Intitulé du projet':'Project title'}</th>
                      <th style={{ padding:'8px 12px', color:'#fff', fontWeight:600, textAlign:'center', fontSize:12, width:120 }}>{lang==='fr'?'Affectation (%)':'Assignment (%)'}</th>
                    ...
                    {affectRows.map((row, i) => (
                      <tr key={i} style={{ borderTop:`0.5px solid ${C.g3}`, background:i%2===0?'#fff':C.g1 }}>
                        <td style={{ padding:'7px 10px' }}>
                          <input value={row.num_as||''} onChange={e=>{const n=[...affectRows];n[i]={...n[i],num_as:e.target.value};setAffectRows(n)}}
                            placeholder="AS-XXXX"
                            style={{ width:'100%', padding:'6px 8px', border:`1px solid ${C.g3}`, borderRadius:6, fontSize:12, outline:'none', boxSizing:'border-box' }}/>
                        </td>
                        <td style={{ padding:'7px 10px' }}>
                          <input value={row.intitule||''} onChange={e=>{const n=[...affectRows];n[i]={...n[i],intitule:e.target.value};setAffectRows(n)}}
                            placeholder={lang==='fr'?'Intitulé du projet':'Project title'}
                            style={{ width:'100%', padding:'6px 8px', border:`1px solid ${C.g3}`, borderRadius:6, fontSize:12, outline:'none', boxSizing:'border-box' }}/>
                        </td>
                        <td style={{ padding:'7px 10px', textAlign:'center' }}>
                          <div style={{ display:'flex', alignItems:'center', gap:6, justifyContent:'center' }}>
                            <input type="number" value={row.pourcentage||''} min={0} max={100}
                              ...
                              style={{ width:60, padding:'6px 8px', border:`1px solid ${C.g3}`, borderRadius:6, fontSize:12, textAlign:'center', outline:'none' }}/>
                            <span style={{ fontSize:13, color:C.gt }}>%</span>
                          </div>
                        </td>
                        <td style={{ padding:'6px', textAlign:'center' }}>
                          <button onClick={()=>setAffectRows(affectRows.filter((_,j)=>j!==i))}
                            style={{ background:'transparent', border:'none', color:C.red, cursor:'pointer', fontSize:15, padding:'2px 6px' }}>✕</button>
                        </td>
                      </tr>
                    ))}
                    {affectRows.length === 0 && (
                      <tr><td colSpan={4} style={{ padding:'20px', textAlign:'center', color:C.gt, fontSize:12 }}>
                        {lang==='fr'?'Cliquer "+ Ajouter un projet" pour commencer':'Click "+ Add project" to start'}
                      </td></tr>
                    )}
                  </tbody>
                </table>
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:10 }}>
                <button onClick={()=>setAffectRows([...affectRows, {num_as:'',intitule:'',pourcentage:''}])}
                  disabled={totalAffect>=100}
                  style={{ padding:'8px 16px', background:totalAffect>=100?C.g3:C.blue, color:totalAffect>=100?C.gt:'#fff', border:'none', borderRadius:8, fontSize:12, fontWeight:600, cursor:totalAffect>=100?'not-allowed':'pointer' }}>
                  + {lang==='fr'?'Ajouter un projet':'Add project'}
                  {totalAffect < 100 && <span style={{ marginLeft:6, fontWeight:400, fontSize:11 }}>({100-totalAffect}% {lang==='fr'?'restant':'remaining'})</span>}
                </button>
                {totalAffect === 100 && <span style={{ fontSize:13, color:C.green, fontWeight:700 }}>✅ {lang==='fr'?'Affectation complète':'Full assignment'}</span>}
                {notComplete && totalAffect > 0 && <span style={{ fontSize:12, color:C.amber, fontWeight:600 }}>⚠ {100-totalAffect}% {lang==='fr'?'restant à affecter':'still to assign'}</span>}
                {overLimit && <span style={{ fontSize:12, color:C.red, fontWeight:600 }}>⛔ {lang==='fr'?`Dépassement de ${totalAffect-100}%`:`${totalAffect-100}% over limit`}</span>}
              </div>
            </div>
          )}
